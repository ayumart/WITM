...
# To upload CSV file
#witm <- read.csv2("Data/WITM_csv_0716.csv")

# To upload CSV file
witm<- read.csv2("Data/WITM_variables.csv")
...
#Convertir campos vacíos de la variable q5 en NA
 ...
 q13_q5 <- base %>%
  ...
  filter(!is.na(q13_ext_funding) & !is.na(q5_var)) %>%
...
```{r registration, echo=F, message=F, warning=F}
...
q13_q9 <- base %>%
  filter(!is.na(q13_ext_funding) & !is.na(q9_agrup)) %>%
  group_by(q13_ext_funding, q9_agrup) %>%
  summarise(count = n(), .groups = 'drop') %>%
  group_by(q9_agrup) %>%
  mutate(total_count = sum(count),
         Percentage = round((count / total_count) * 100, 2)) %>%
...
form_2000<- base %>%
  filter(!is.na(q13_ext_funding) & q9_agrup=="Before 2000") %>%
  group_by(q9_agrup) %>% 
  summarise(
    Total = n(),
    Yes = sum(q13_ext_funding == "yes", na.rm = TRUE),
...
Of the total organizations that received external funding in 2023, `r primary_focus1`%  promote the rights of women or girls or trans or gender diverse people with intersecting identities, and this is the “primary focus of all our work”.


```{r data4, echo=F, message=F, warning=F}
knitr::kable(funding_q1) %>% 
  kable_minimal()
```


```{r fundingb, echo=F, message=F, warning=F}
funding_q2 <- base %>%
  mutate(q1_var = case_when(
    q1_description == "primary_focus" ~ "Primary focus",
    q1_description == "specific_projects" ~ "Specific project(s)",
...
funding_q5 <- base %>%
  ...
  filter(q13_ext_funding == "yes" & q14_funding_annual_budget_2023 > 0 & !is.na(q5_var)) %>%
  group_by(q5_var) %>%
  summarise(count = n(), .groups = 'drop') %>%
  mutate(total_count = sum(count),  # Calcula el total de conteos
         Percentage = round((count / total_count) * 100, 2)) %>%  # Calcula el porcentaje
...
funding_q9 <- base %>%
  filter(q13_ext_funding == "yes" & q14_funding_annual_budget_2023 > 0 & !is.na(q9_agrup)) %>%
  group_by(q9_agrup) %>%
  summarise(n = n(), .groups = 'drop') %>%
  mutate(total_count = sum(n),  # Calcula el total de conteos
         Percentage = round((n / total_count) * 100, 2)) %>%  # Calcula el porcentaje
...
When examining this data by organizational focus, we observe that organizations primarily dedicated to the rights of women, girls, trans, and gender-diverse individuals see an even greater increase in feminist funding.

```{r q15r, echo=F, message=F, warning=F}
knitr::kable(q15_q1) %>% 
  kable_minimal()
...
q15_q9 <- base %>%
  filter(!is.na(q9_agrup) & !is.na(q15_key_sources)) %>% 
  group_by(q9_agrup) %>%
  summarise(Total = n(),
    multirateral = sum(q15_key_sources.multilateral_funders == 1),
    bilateral = sum(q15_key_sources.bilateral_funders == 1),
    ...
    Government = round(gov / Total*100, 1),
    Other = round(other / Total*100, 1)) %>% 
    select("Formation period"=q9_agrup,2,12:20)
...
```{r q18_descr, echo=F, message=F, warning=F}
...
```{r q18a, echo=F, message=F, warning=F}

q18_5 <- base %>%
  filter(!is.na(q5_registered) & (q18_new_funders.not_new_funders!=1)) %>%
  group_by(q5_registered) %>% 
  summarise(
    Total = (round(n(),0)),
    multilateral = sum(q18_new_funders.multilateral_funders == 1, na.rm = TRUE),
  ...
  mutate(Registration=recode(Registration, "98"="Other", "n_registered"="No registered", "y_registered"="Registered"))
...
q18_9 <- base %>%
    filter(!is.na(q9_agrup) & (q18_new_funders.not_new_funders!=1)) %>%
  group_by(q9_agrup) %>% 
  summarise(
    Total = (round(n(),0)),
    multilateral = sum(q18_new_funders.multilateral_funders == 1, na.rm = TRUE),
...
```{r q19a, echo=F, message=F, warning=F}
...
```{r q19d, echo=F, message=F, warning=F}

q19_q5 <- base %>%
  filter(!is.na(q19_lose_funding) & !is.na(q5_registered)) %>%
  group_by(q5_registered) %>% 
  summarise(
    Total = (round(n(),0)),
    no=sum(q19_lose_funding.no==1, na.rm=TRUE),
...
q19_yes_q5<-base %>%
  filter(q19_lose_funding.no!=1 & !is.na(q5_registered)) %>% 
  group_by(q5_registered) %>% 
  summarise(Total= round(n(),0),
    multilateral = sum(q19_lose_funding.y_multilateral_funders == 1, na.rm = TRUE),
    bilateral = sum(q19_lose_funding.y_bilateral_funders == 1, na.rm = TRUE),
...
q19_q9 <- base %>%
  mutate(q9_agrup2=ifelse((q9_agrup=="Year 2023" | q9_agrup=="Year 2022" | q9_agrup=="Year 2021"),"Between 2021 and 2023", q9_agrup)) %>% 
  filter(!is.na(q19_lose_funding) & !is.na(q9_agrup2)) %>%
  group_by(q9_agrup2) %>% 
  summarise(
    Total = (round(n(),0)),
    no=sum(q19_lose_funding.no==1, na.rm=TRUE),
    ...
    "No funding lost"= round(no/Total*100,1),
    "Yes, lost funding"= round(yes/Total*100,1)) %>% 
  select("Formation period"="q9_agrup2",2,5:6)


q19_yes_q9<-base %>%
    mutate(q9_agrup2=ifelse((q9_agrup=="Year 2023" | q9_agrup=="Year 2022" | q9_agrup=="Year 2021"),"Between 2021 and 2023", q9_agrup)) %>% 
  filter(q19_lose_funding.no!=1 & !is.na(q9_agrup)) %>% 
  group_by(q9_agrup2) %>% 
  summarise(Total= round(n(),0),
    multilateral = sum(q19_lose_funding.y_multilateral_funders == 1, na.rm = TRUE),
    bilateral = sum(q19_lose_funding.y_bilateral_funders == 1, na.rm = TRUE),
    ...
    Government = round(gov / Total * 100, 1),
    Other = round(other / Total * 100, 1)) %>% 
  select("Formation period"="q9_agrup2",2,12:20) %>% 
    arrange(desc(Total))
...
```{r q20a, echo=F, message=F, warning=F}
...
```{r q20b, echo=F, message=F, warning=F}

q20_q5 <- base %>%
  filter(q19_lose_funding!=1 & !is.na(q20_loss_impact) & !is.na(q5_registered)) %>%
  group_by(q5_registered) %>% 
  summarise(
    Total = (round(n(),0)),
    no=sum(q20_loss_impact.no_impact==1, na.rm=TRUE),
...
q20_q9 <- base %>%
  mutate(q9_agrup2=ifelse((q9_agrup=="Year 2023" | q9_agrup=="Year 2022" | q9_agrup=="Year 2021"),"Between 2021 and 2023", q9_agrup)) %>% 
  filter(q19_lose_funding!=1 & !is.na(q20_loss_impact) & !is.na(q9_agrup2)) %>%
  group_by(q9_agrup2) %>% 
  summarise(
    Total = (round(n(),0)),
    no=sum(q20_loss_impact.no_impact==1, na.rm=TRUE),
    ...
    "Staff faced a salary gap"= round(salary/Total*100,1),
    "Other"= round(other/Total*100,1)) %>% 
  select("Formation period"="q9_agrup2",2,9:14)

...
# 1.	Lesbian, bisexual and queer rights + trans and non-binary rights + intersex rights = LBQTI+
# 2.	Young feminist + girls’ movements 
# 3.	Sex workers rights
# 4.	Religious & ethnic minority rights + anti-caste
# 5.	Climate and environmental justice 
# 6.	Countering anti-gender & anti-rights 
# 7.	Anti-militarization and peace + crisis response 
# 8.	Harm reduction + resisting the war on drugs
...
witm <- witm %>% rename(q2_description=q1_description)
witm <- witm %>% rename(q2_description_group=q1_description_group)

witm <- witm %>% rename(q3_name=q2_name)

write.csv(witm, "Data/WITM_cleaned_20240912.csv",row.names=FALSE)

...
archivo <- "cuadros/univariado.xlsx"
...
print(paste("Number of final answers:",q0$Count))
```
...
q1<-base %>%
  ...
  mutate(q1_description=forcats::fct_recode(as_factor(q1_description),
                                            "a.Primary focus of all our work" = "primary_focus",
...
```{r excel, eval=F, echo=F, message=F, warning=F}

write.xlsx(q1, file = archivo, sheetName="q1")
...
```{r excel2, eval=F, echo=F, message=F, warning=F}

q4 <- loadWorkbook(archivo)
addWorksheet(q4, sheetName = "q4")
writeData(q4, sheet = "q4", x = q4_data)
saveWorkbook(q4, archivo, overwrite = TRUE)
```
...
```{r excel3, eval=F, echo=F, message=F, warning=F}

q4_f <- loadWorkbook(archivo)
addWorksheet(q4_f, sheetName = "q4_focus")
writeData(q4_f, sheet = "q4_focus", x = q4_focus)
saveWorkbook(q4_f, archivo, overwrite = TRUE)
```
...
** Forms by organizing grouped by spotlight themes/groups**
...
q4_agrup<-base %>%
  ...
  summarise(Total= round(n(),0),
    ...
    nin = sum(q4_awid_nin == 1, na.rm = TRUE),
    climate = sum(q4_awid_climate == 1, na.rm = TRUE),
    antig = sum(q4_awid_antig == 1, na.rm = TRUE),
    crisis = sum(q4_awid_crisis == 1, na.rm = TRUE),
    ...
    "Religious & ethnic minority rights + anti-caste"  = round(nin / Total * 100, 1),
    "Climate and environmental justice " = round(climate/ Total * 100, 1),
    "Countering anti-gender & anti-rights " = round(antig / Total * 100, 1),
    "Anti-militarization and peace + crisis response"  = round(crisis / Total * 100, 1),
...
```{r excel4, eval=F, echo=F, message=F, warning=F}
...
q4_agrup1<-base %>%
  ...
  summarise(Total= round(n(),0),
    ...
    young = sum(q4_awid_young == 1, na.rm = TRUE),
    sex = sum(q4_awid_sex == 1, na.rm = TRUE),
    monirity = sum(q4_awid_nin == 1, na.rm = TRUE),
    climate = sum(q4_awid_climate == 1, na.rm = TRUE),
    antigender = sum(q4_awid_antig == 1, na.rm = TRUE),
    crisis = sum(q4_awid_crisis == 1, na.rm = TRUE),
    harm = sum(q4_awid_harm == 1, na.rm = TRUE),
    "LBQTI+" = round(lgbtiq/ Total * 100, 1)) %>% 
...
q4_group1 <- loadWorkbook(archivo)
addWorksheet(q4_group1, sheetName = "q4_group")
writeData(q4_group1, sheet = "q4_group", x = q4_agrup1)
saveWorkbook(q4_group1, archivo, overwrite = TRUE)
```
____
...
```{r excel5, eval=F, echo=F, message=F, warning=F}

q5_d <- loadWorkbook(archivo)
addWorksheet(q5_d, sheetName = "q5")
writeData(q5_d, sheet = "q5", x = q5)
saveWorkbook(q5_d, archivo, overwrite = TRUE)
```
____
...
```{r excel6, eval=F, echo=F, message=F, warning=F}

q6_d <- loadWorkbook(archivo)
addWorksheet(q6_d, sheetName = "q6")
writeData(q6_d, sheet = "q6", x = q6)
saveWorkbook(q6_d, archivo, overwrite = TRUE)
```
...
```{r region2, echo=F, message=F, warning=F}
...
```{r excel7, eval=F, echo=F, message=F, warning=F}

q7_d <- loadWorkbook(archivo)
addWorksheet(q7_d, sheetName = "q7")
writeData(q7_d, sheet = "q7", x = q7_final)
saveWorkbook(q7_d, archivo, overwrite = TRUE)


q7_region <- loadWorkbook(archivo)
addWorksheet(q7_region, sheetName = "q7_group")
writeData(q7_region, sheet = "q7_group", x = region_summary)
saveWorkbook(q7_region, archivo, overwrite = TRUE)
...
```
```{r excel8, eval=F, echo=F, message=F, warning=F}

q8_d <- loadWorkbook(archivo)
addWorksheet(q8_d, sheetName = "q8")
writeData(q8_d, sheet = "q8", x = q8)
saveWorkbook(q8_d, archivo, overwrite = TRUE)
```
____
...
```{r excel9, eval=F, echo=F, message=F, warning=F}

q9_d <- loadWorkbook(archivo)
addWorksheet(q9_d, sheetName = "q9")
writeData(q9_d, sheet = "q9", x = q9)
saveWorkbook(q9_d, archivo, overwrite = TRUE)
```
...
```{r excel10, eval=F, echo=F, message=F, warning=F}

q10_d <- loadWorkbook(archivo)
addWorksheet(q10_d, sheetName = "q10")
writeData(q10_d, sheet = "q10", x = q10_grouped)
saveWorkbook(q10_d, archivo, overwrite = TRUE)
```
...
```{r excel11, eval=F, echo=F, message=F, warning=F}

q11_d <- loadWorkbook(archivo)
addWorksheet(q11_d, sheetName = "q11")
writeData(q11_d, sheet = "q11", x = q11)
saveWorkbook(q11_d, archivo, overwrite = TRUE)
```
...
```{r excel13, eval=F, echo=F, message=F, warning=F}

q13_d <- loadWorkbook(archivo)
addWorksheet(q13_d, sheetName = "q13")
writeData(q13_d, sheet = "q13", x = q13)
saveWorkbook(q13_d, archivo, overwrite = TRUE)
```
...
```{r excel14, eval=F, echo=F, message=F, warning=F}
...
#Unir los dataframes
q14_combinedb <- q14_2021b %>%
  full_join(q14_2022b, by = "q14") %>%
  full_join(q14_2023b, by = "q14")

q14_groupedb<-q14_combinedb %>% 
  rename("Percentage of External Funding in Total Annual Budget"=q14)



q14_d <- loadWorkbook(archivo)
addWorksheet(q14_d, sheetName = "q14")
writeData(q14_d, sheet = "q14", x = q14_groupedb)
saveWorkbook(q14_d, archivo, overwrite = TRUE)
```
...
```{r excel15, eval=F, echo=F, message=F, warning=F}
...
q15_d <- loadWorkbook(archivo)
addWorksheet(q15_d, sheetName = "q15")
writeData(q15_d, sheet = "q15", x = q15b)
saveWorkbook(q15_d, archivo, overwrite = TRUE)
```
...
```{r excel18, eval=F, echo=F, message=F, warning=F}
...
q18_d <- loadWorkbook(archivo)
addWorksheet(q18_d, sheetName = "q18")
writeData(q18_d, sheet = "q18", x = q18b)
saveWorkbook(q18_d, archivo, overwrite = TRUE)
```
...
```{r excel19, eval=F, echo=F, message=F, warning=F}
...
q19_d <- loadWorkbook(archivo)
addWorksheet(q19_d, sheetName = "q19")
writeData(q19_d, sheet = "q19", x = q19b)
saveWorkbook(q19_d, archivo, overwrite = TRUE)
```
...
```{r excel20, eval=F, echo=F, message=F, warning=F}
...
q20_d <- loadWorkbook(archivo)
addWorksheet(q20_d, sheetName = "q20")
writeData(q20_d, sheet = "q20", x = q20b)
saveWorkbook(q20_d, archivo, overwrite = TRUE)
```
...
```{r excel22, eval=F, echo=F, message=F, warning=F}


q22_d <- loadWorkbook(archivo)
addWorksheet(q22_d, sheetName = "q22")
writeData(q22_d, sheet = "q22", x = q22)
saveWorkbook(q22_d, archivo, overwrite = TRUE)
```
...
```{r excel23, eval=F, echo=F, message=F, warning=F}
#Convertir campos vacíos de la variable q23 en NA
 base <- base %>%
  mutate(q23_context_emergency = na_if(q23_context_emergency, ""))
...
q23b <- base %>%
  ...
  summarise(
    Total = (round(n(),0)),
    "Outbreak of conflict, war and violence"=sum(q23_context_emergency.conflict_war_violence==1, na.rm=TRUE),
    "COVID-19 pandemic"=sum(q23_context_emergency.covid19==1, na.rm = TRUE),
    "Other health-related emergencies"=sum(q23_context_emergency.health_emergencies==1, na.rm = TRUE),
    "Climate crisis-related disaster"=sum(q23_context_emergency.climate_disaster==1, na.rm = TRUE),
    "Crack-down on civic space and anti-rights backlash"=sum(q23_context_emergency.civic_crackdown==1, na.rm = TRUE),
    "Physical security threats"=sum(q23_context_emergency.physical_threats==1, na.rm=TRUE),
    "Digital security threats"=sum(q23_context_emergency.digital_threats==1, na.rm=TRUE),
    "Holistic safety, protection or collective care"=sum(q23_context_emergency.Holistic_safety._protection_or_collective_care_for_staff_or_key_partners==1, na.rm=TRUE),
    "Other"=sum(q23_context_emergency.98==1, na.rm=TRUE)) %>% 
      pivot_longer(cols = everything(),
               names_to = "Source",
               values_to = "N")
...
q23_d <- loadWorkbook(archivo)
addWorksheet(q23_d, sheetName = "q23")
writeData(q23_d, sheet = "q23", x = q23b)
saveWorkbook(q23_d, archivo, overwrite = TRUE)
```
...
```{r excel25, eval=F, echo=F, message=F, warning=F}
q25_d <- loadWorkbook(archivo)
addWorksheet(q25_d, sheetName = "q25")
writeData(q25_d, sheet = "q25", x = q25)
saveWorkbook(q25_d, archivo, overwrite = TRUE)
```
...
ggplot(q26_yes, aes(x = reorder(Source, -Percentage), y = Percentage, fill=Source)) +
  ...
  theme_minimal() +
  theme(legend.position = "none",
  panel.grid.major.x = element_blank(), # Elimina líneas de la cuadrícula vertical
    panel.grid.minor.x = element_blank(), # Elimina líneas de la cuadrícula vertical menores
    panel.grid.major.y = element_blank(), # Elimina líneas de la cuadrícula horizontal
...
```{r excel26, eval=F, echo=F, message=F, warning=F}

q26b<-base %>%
  filter(!is.na(q26_new_counter_funder)) %>%
  summarise(Total= round(n(),0),
    multilateral = sum(q26_new_counter_funder.multilateral_funders == 1, na.rm = TRUE),
    bilateral = sum(q26_new_counter_funder.bilateral_funders == 1, na.rm = TRUE),
    philanthropic = sum(q26_new_counter_funder.philanthropic_foundations == 1, na.rm = TRUE),
...
```{r excel27, eval=F, echo=F, message=F, warning=F}
...
q27_d <- loadWorkbook(archivo)
addWorksheet(q27_d, sheetName = "q27")
writeData(q27_d, sheet = "q27", x = q27b)
saveWorkbook(q27_d, archivo, overwrite = TRUE)
```
...
```{r excel28, eval=F, echo=F, message=F, warning=F}
...
q28_d <- loadWorkbook(archivo)
addWorksheet(q28_d, sheetName = "q28")
writeData(q28_d, sheet = "q28", x = q28b)
saveWorkbook(q28_d, archivo, overwrite = TRUE)
```
_____
...
```{r excel29, eval=F, echo=F, message=F, warning=F}


q29_d <- loadWorkbook(archivo)
addWorksheet(q29_d, sheetName = "q29")
writeData(q29_d, sheet = "q29", x = q29)
saveWorkbook(q29_d, archivo, overwrite = TRUE)
```
...
```{r excel30, eval=F, echo=F, message=F, warning=F}


q30_d <- loadWorkbook(archivo)
addWorksheet(q30_d, sheetName = "q30")
writeData(q30_d, sheet = "q30", x = q30)
saveWorkbook(q30_d, archivo, overwrite = TRUE)
```
_____
...
```{r excel31, eval=F, echo=F, message=F, warning=F}


q31b<-base %>%
  filter(!is.na(q31_funders_practices)) %>%
  summarise(Total= round(n(),0),
    "Simplified funding application processes"  = sum(q31_funders_practices.funding_application_simplified == 1, na.rm = TRUE),
    "Core funding" = sum(q31_funders_practices.core_funding == 1, na.rm = TRUE),
    "Multi-year funding" = sum(q31_funders_practices.multi_year_funding == 1, na.rm = TRUE),
    "Flexible funding" = sum(q31_funders_practices.flexible_funding == 1, na.rm = TRUE),
   "Open communication"  = sum(q31_funders_practices.open_communication_on_funding == 1, na.rm = TRUE),
    "Meaningful consultations to set funders agendas and priorities" = sum(q31_funders_practices.meaningful_funder_consultations == 1, na.rm = TRUE),
     "Funders alignment and trust with grantee partner’s political values and missions" = sum(q31_funders_practices.funders_alignment_with_values == 1, na.rm = TRUE),
    "Simplified reporting processes"  = sum(q31_funders_practices.simplified_reporting== 1, na.rm = TRUE),
    "Introduction of a participatory grant decision-making process" = sum(q31_funders_practices.participatory_grantmaking== 1, na.rm = TRUE),
    "All of the above" = sum(q31_funders_practices.all== 1, na.rm = TRUE),
    "None of the above"  = sum(q31_funders_practices.none== 1, na.rm = TRUE),
    Other = sum(q31_funders_practices.98== 1, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(),
             names_to = "Source",
             values_to = "N")
...
q31_d <- loadWorkbook(archivo)
addWorksheet(q31_d, sheetName = "q31")
writeData(q31_d, sheet = "q31", x = q31b)
saveWorkbook(q31_d, archivo, overwrite = TRUE)
```
_____
...
```{r excel32, eval=F, echo=F, message=F, warning=F}
...
q32a1<-base %>% 
  ...
  group_by(q32_barriers_few) %>% 
  summarise(Total=n()) %>% 
  rename("q32"=1, "We are skilled in resource mobilization and face relatively few difficulties to resource our work"=2)
...
q32b1<-base %>% 
  ...
  group_by(q32_barriers_lack_information) %>% 
  summarise(Total=n()) %>% 
  rename("q32"=1, "We don’t have enough information on where, from whom and what funding is actually available"=2)
...
q32c1<-base %>% 
  ...
  group_by(q32_barriers_capacity) %>% 
  summarise(Total=n()) %>% 
  rename("q32"=1, "We don't have the staff capacity and/or experience to apply for funding"=2)
...
q32d1<-base %>% 
  ...
  group_by(q32_barriers_complexity) %>% 
  summarise(Total=n()) %>% 
  rename("q32"=1, "Funding processes are complex, inaccessible and unrealistic"=2)
...
q32e1<-base %>% 
  ...
  group_by(q32_barriers_unattainable) %>% 
  summarise(Total=n()) %>% 
  select("q32"=1, "Monitoring and evaluation processes are extremely technical and far from the reality of feminist organizing"=2)
...
q32f1<-base %>% 
  ...
  group_by(q32_barriers_legal) %>% 
  summarise(Total=n()) %>% 
  select("q32"=1, "We often don't meet funders’ legal, judicial and/or administrative requirements to be eligible for funding"=2)
...
q32g1<-base %>% 
  ...
  group_by(q32_barriers_political) %>% 
  summarise(Total=n()) %>% 
  select("q32"=1, "We work in regions and/or on issues that are criminalized by our governments and face restrictions and/or threats to receiving foreign funding"=2)
...
q32h1<-base %>% 
  ...
  group_by(q32_barriers_different_strategies) %>% 
  summarise(Total=n()) %>% 
  select("q32"=1, "We are under pressure to comply with funders political strategies, including not engaging with certain agendas and/or in certain regions"=2)
...
q32i1<-base %>% 
  ...
  group_by(q32_barriers_unavailable) %>% 
  summarise(Total=n()) %>% 
  select("q32"=1, "There is very little funding available for the movements, agendas and/ or countries & regions we  work with"=2)
...
q32j1<-base %>% 
  ...
  group_by(q32_barriers_little_transparency) %>% 
  summarise(Total=n()) %>% 
  select("q32"=1, "There is very little transparency about funders’ priorities, assessment criteria and who receives funding in the end"=2)



#Unir los dataframes
q32_combined1 <- q32a1 %>%
  full_join(q32b1, by = "q32") %>% 
  full_join(q32c1, by = "q32") %>% 
  full_join(q32d1, by = "q32") %>% 
  full_join(q32e1, by = "q32") %>% 
  full_join(q32f1, by = "q32") %>% 
  full_join(q32g1, by = "q32") %>% 
  full_join(q32h1, by = "q32") %>% 
  full_join(q32i1, by = "q32") %>% 
  full_join(q32j1, by = "q32")




# Transformar los datos a formato largo
q32_long1 <- q32_combined1 %>%
  pivot_longer(cols = -q32, names_to = "Response", values_to = "N")

q32_wide1 <- q32_long1 %>%
  pivot_wider(
    names_from = q32,  # Usa las categorías de 'Response' como nombres de las columnas
    values_from = N # Usa los valores de 'Percentage' para llenar las columnas
  )


q32_d <- loadWorkbook(archivo)
addWorksheet(q32_d, sheetName = "q32")
writeData(q32_d, sheet = "q32", x = q32_wide1)
saveWorkbook(q32_d, archivo, overwrite = TRUE)
```
...
```{r excel33, eval=F, echo=F, message=F, warning=F}


q33b<-base %>%
  filter(!is.na(q33_awid_support)) %>%
  summarise(Total= round(n(),0),
    "Information on fundraising sources"  = sum(q33_awid_support.fundraising_info_database == 1, na.rm = TRUE),
    "Research on the resourcing realities of feminist organizing" = sum(q33_awid_support.resourcing_research == 1, na.rm = TRUE),
    "Facilitate connections and networks with feminist activists globally" = sum(q33_awid_support == 1, na.rm = TRUE),
    "Create more open spaces for dialogue with funders" = sum(q33_awid_support.open_dialogue_funders == 1, na.rm = TRUE),
    "Support your advocacy with funders for more responsive resourcing" = sum(q33_awid_support.advocacy_support_resourcing == 1, na.rm = TRUE),
    "Support your advocacy with national governments and regional bodies" = sum(q33_awid_support.advocacy_support_gov_bodies == 1, na.rm = TRUE),
    "Support skills sharing on alternative ways of feminist resourcing" = sum(q33_awid_support.resourcing_skill_sharing == 1, na.rm = TRUE),
    "Support  to create better bridges with women's and feminist funds" = sum(q33_awid_support.bridges_feminist_funds== 1, na.rm = TRUE),
    Other = sum(q33_awid_support.98== 1, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(),
             names_to = "Source",
             values_to = "N")
...
q33_d <- loadWorkbook(archivo)
addWorksheet(q33_d, sheetName = "q33")
writeData(q33_d, sheet = "q33", x = q33b)
saveWorkbook(q33_d, archivo, overwrite = TRUE)
```
...
```{r excel36, eval=F, echo=F, message=F, warning=F}
...
#Unir los dataframes
q36_combinedb <- q36_2025b %>%
  full_join(q36_2026b, by = "q36")


q36_d <- loadWorkbook(archivo)
addWorksheet(q36_d, sheetName = "q36")
writeData(q36_d, sheet = "q36", x = q36_combinedb)
saveWorkbook(q36_d, archivo, overwrite = TRUE)
```
_____
...
```{r excel37, eval=F, echo=F, message=F, warning=F}

q37_d <- loadWorkbook(archivo)
addWorksheet(q37_d, sheetName = "q37")
writeData(q37_d, sheet = "q37", x = q37)
saveWorkbook(q37_d, archivo, overwrite = TRUE)
```
_____
...
```{r excel38, eval=F, echo=F, message=F, warning=F}
q38b<-base %>% 
  mutate(q38_closing_risk=case_when(
    q38_closing_risk=="yes" ~ "Yes",
    q38_closing_risk=="no" ~ "No",
...
q38_d <- loadWorkbook(archivo)
addWorksheet(q38_d, sheetName = "q38")
writeData(q38_d, sheet = "q38", x = q38b)
saveWorkbook(q38_d, archivo, overwrite = TRUE)
```
...
ggplot(q39, aes(x = reorder(Benefits, Percentage), y = Percentage, fill = Benefits)) +
  ...
  panel.grid.major.x = element_blank(), # Elimina líneas de la cuadrícula vertical
    panel.grid.minor.x = element_blank(), # Elimina líneas de la cuadrícula vertical menores
    panel.grid.major.y = element_blank(), # Elimina líneas de la cuadrícula horizontal
    panel.grid.minor.y = element_blank()) + # Elimina líneas de la cuadrícula horizontal menores 
  coord_flip()
...
```{r excel39, eval=F, echo=F, message=F, warning=F}
...
q39_d <- loadWorkbook(archivo)
addWorksheet(q39_d, sheetName = "q39")
writeData(q39_d, sheet = "q39", x = q39b)
saveWorkbook(q39_d, archivo, overwrite = TRUE)
```
...
```{r excel40, eval=F, echo=F, message=F, warning=F}

q40_d <- loadWorkbook(archivo)
addWorksheet(q40_d, sheetName = "q40")
writeData(q40_d, sheet = "q40", x = q40)
saveWorkbook(q40_d, archivo, overwrite = TRUE)
```
...
```{r excel41, eval=F, echo=F, message=F, warning=F}

q41_d <- loadWorkbook(archivo)
addWorksheet(q41_d, sheetName = "q41")
writeData(q41_d, sheet = "q41", x = q41)
saveWorkbook(q41_d, archivo, overwrite = TRUE)
```
...
```{r excel42, eval=F, echo=F, message=F, warning=F}

q42_d <- loadWorkbook(archivo)
addWorksheet(q42_d, sheetName = "q42")
writeData(q42_d, sheet = "q42", x = q42)
saveWorkbook(q42_d, archivo, overwrite = TRUE)
```
...
```{r excel45, eval=F, echo=F, message=F, warning=F}

q45_d <- loadWorkbook(archivo)
addWorksheet(q45_d, sheetName = "q45")
writeData(q45_d, sheet = "q45", x = q45)
saveWorkbook(q45_d, archivo, overwrite = TRUE)
```
...
```{r excel46, eval=F, echo=F, message=F, warning=F}

q46_d <- loadWorkbook(archivo)
addWorksheet(q46_d, sheetName = "q46")
writeData(q46_d, sheet = "q46", x = q46)
saveWorkbook(q46_d, archivo, overwrite = TRUE)
```
_____
...
```{r excel47, eval=F, echo=F, message=F, warning=F}

q47_d <- loadWorkbook(archivo)
addWorksheet(q47_d, sheetName = "q47")
writeData(q47_d, sheet = "q47", x = q47)
saveWorkbook(q47_d, archivo, overwrite = TRUE)
```
